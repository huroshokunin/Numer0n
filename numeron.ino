...
static bool isMading = false;   //現在数字列を作っているかどうか、falseならmake_num()関数終了
static bool isGuessing = false; //現在数字列を推測中かどうか,falseならguess_num()関数終了
...
void make_num(){
  ...
  while(isMading){
    ...
    if((sw0 == SW_PUSH) && (sw1 == SW_ON)){
      //数字の重複が無ければ数字列決定
      if((correctNum[0]!=correctNum[1]) && (correctNum[0]!=correctNum[2]) && (correctNum[1] != correctNum[2])){
        isMading = false;
        all_disp_zero();
        return;
      }
    ...
    if((sw0 == SW_ON) && (sw1 == SW_PUSH)){
        correctNum[usingDisp] = (correctNum[usingDisp] == 0) ? 9 : correctNum[usingDisp]-1;
...
void guess_num(){
  ...
  while(isGuessing){
    ...
    if((sw0 == SW_PUSH) && (sw1 == SW_ON)){
      //数字の重複が無ければ、数字列決定の処理へ
      if((guessedNum[0]!=guessedNum[1]) && (guessedNum[0]!=guessedNum[2]) && (guessedNum[1] != guessedNum[2])){
        isGuessing = false;
        all_disp_zero();
    ...
    //sw0を押した後にsw1を押したときの処理
        ...
        guessedNum[usingDisp] = (guessedNum[usingDisp] == 0) ? 9 : guessedNum[usingDisp]-1;
        //数字の重複が無ければ、数字列決定の処理へ
        if((guessedNum[0]!=guessedNum[1]) && (guessedNum[0]!=guessedNum[2]) && (guessedNum[1] != guessedNum[2])){
          isGuessing = false;
          all_disp_zero();
